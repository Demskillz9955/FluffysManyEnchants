	...
	public static class Blessing {
		...
		public static void DolphinBless(PlayerTickEvent event) {
			...
			if (player.isInWater() == true) {
				EnchantmentUtils.AddEffectBasedOnLevelEntity(player, Enchantments.DOLPHIN_BLESSED.get(), 10, 1,
						Effects.DOLPHINS_GRACE, EquipmentSlotType.FEET);

			}
		}

		public static int GetCurrentEnchantment(PlayerEntity player) {
			return EnchantmentHelper.getEnchantmentLevel(Enchantments.DOLPHIN_BLESSED.get(),
					player.getItemStackFromSlot(EquipmentSlotType.MAINHAND));
		}
