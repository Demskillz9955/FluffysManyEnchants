	   ...
	   public boolean canVillagerTrade() {
		   ...
		   public static class MinersDelight{
				...
				public static void Delight(RightClickBlock event) {
					...
					if(worldIn.getBlockState(event.getPos().down()) != Blocks.AIR.getDefaultState() || worldIn.getBlockState(event.getPos().east()) != Blocks.AIR.getDefaultState() || worldIn.getBlockState(event.getPos().west()) != Blocks.AIR.getDefaultState() 
							|| worldIn.getBlockState(event.getPos().north()) != Blocks.AIR.getDefaultState() || worldIn.getBlockState(event.getPos().south()) != Blocks.AIR.getDefaultState()) {
					
						
						if (player.hasItemInSlot(EquipmentSlotType.MAINHAND) 
								
								&& EnchantmentHelper.getEnchantmentLevel(Enchantments.MINERS_DELIGHT.get(),player.getItemStackFromSlot(EquipmentSlotType.MAINHAND)) > 0) {
					...
					EnchantmentUtils.DamageItemBasedOnEnchantmentLevel(30, player, EquipmentSlotType.MAINHAND, Enchantments.MINERS_DELIGHT.get(), 1);
					EnchantmentUtils.DamageItemBasedOnEnchantmentLevel(20, player, EquipmentSlotType.MAINHAND, Enchantments.MINERS_DELIGHT.get(), 2);
					EnchantmentUtils.DamageItemBasedOnEnchantmentLevel(15, player, EquipmentSlotType.MAINHAND, Enchantments.MINERS_DELIGHT.get(), 3);
